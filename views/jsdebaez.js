...
// Objeto que contiene el grÃ¡fico
let myChart = null
...
document.addEventListener("DOMContentLoaded", function(event){
	...
	// inicializa la ventana mostrando la pestaÃ±a por defecto
	...
	// Carga en todos los botones de control el evento que harÃ¡ que se muestre la pestaÃ±a correcta
	btnControls.forEach(function(element, index){
		document.querySelector( "#btn-" + element[0] ).addEventListener("click", event => {
			...
			// Muestra la pestaÃ±a seleccionada y oculta las otras
			btnControls.forEach(function(btn, i){
...
async function addVisitStation(){
	const response = await fetch("datos.php?chipid=" + chipid + "&mode=visit-station")
...
// Analiza el json, carga los valores y el grÃ¡fico
// =================================
function procesar(datos, addData = true){
	...
	// =================================
	document.querySelector("#temp").innerHTML = datos[0].temperatura.split(".")[0] + "ÂºC";
	...
	document.querySelector("#temp-max").innerHTML = datos[0].tempmax + "ÂºC"
	document.querySelector("#temp-min").innerHTML = datos[0].tempmin + "ÂºC"
	...
	document.querySelector("#sen-max").innerHTML = datos[0].sensamax + "ÂºC"
	document.querySelector("#sen-min").innerHTML = datos[0].sensamin + "ÂºC"
	...
	// direcciÃ³n del viento en visor
	viento__val__veleta.innerHTML = datos[0].veleta;

	// botÃ³n direcciÃ³n del viento
...
function renderCharts(estacion, fecha, itemsGrafico){
	...
	// actualiza el contenido del grÃ¡fico si lo estamos usando sin destruir en cada ocasiÃ³n
	//myChart.update()
